...
import com.example.pmdm02_03actividades5.R
...
fun Actividad2() {
    ...
    Column(
        ...
        Spacer(modifier = Modifier.height(if (showLoading) 30.dp else 0.dp))
...
fun Actividad3() {
    ...
    Column(
        ...
        Row(
            ...
            Button(onClick = { if (progressStatus < 1.0f) {progressStatus += 0.1f} }) {
                Text(text = "Incrementar")
            }

            Spacer(modifier = Modifier.width(10.dp))

            Button(onClick = { if (progressStatus > 0.0f) {progressStatus -= 0.1f} }) {
                Text(text = "Reducir")
            }
        }
    }
...
fun Actividad4() {
    ...
    Box(
        ...
        TextField(
            ...
            onValueChange =
            { newText ->
                val importeText = newText.replace(",", ".")
                if (importeText.count{it == '.'} <= 1) { myVal = importeText }
            },
...
fun Actividad5() {
    ...
    Box(
        ...
        OutlinedTextField(
            ...
            onValueChange =
            { newValue ->
                val validValue = newValue.replace(",", ".")
                if (isValidDecimal(validValue)) { myVal = validValue }
            },
            label = { Text(text = "Importe") },
